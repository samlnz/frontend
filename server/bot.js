...
// IMPORTANT: Railway provides the port via process.env.PORT
...
async function chargeEntries() {
  ...
  for (const p of gameState.participants) {
    try {
      ...
      await fetch(`${FINANCE_API_URL}/api/game/entry`, {
        method: 'POST',
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Star Bingo Engine online on port ${PORT}`));
